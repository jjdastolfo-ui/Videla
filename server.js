...
async function scrapearPrecioMAG(fechaDesde, fechaHasta) {
  ...
  // El endpoint de INMAG arrendamiento acepta fechas por query string
  ...
  try {
    const resp = await fetch(url, {
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; VIDELA-Bot/1.0)",
        "Accept": "text/html,application/xhtml+xml"
      }
    });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const html = await resp.text();
    return parsearTablaMAG(html);
  } catch (err) {
    console.error("Error scrapeando MAG:", err.message);
    return null;
  }
}
